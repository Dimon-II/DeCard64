...
type
  ...
  public
    ...
    procedure EraseDictWord(const AWord: String);
...
begin
  ...
  procedure TSynEditSpellCheck.EraseDictWord (const AWord : String);
var
  sw : String;
begin
  sw:=Trim(AWord);
  if FEnabled and Assigned(FHunspellHandle) and (length(sw)>0) then with FUserDict do if IndexOf(sw)>-1 then begin
    Delete(IndexOf(sw));
